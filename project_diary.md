...
Guillermo sent both Yim and me a .tar with all the length counts instead, and when I tryed the scipt locally it worked. This mean that we have a csv file with every read count between 18 and 36 bp for all 113 samples from the 6 projects. I sent it to Yim and Guillermo. It has some strange numbers however, many 0 and 1 in one of the projects. Guillermo said that it was a comment for this project "Some samples have low RIN value and we wonder if they should be treated with RiboZero prior to library prep".  
...
I have decided to use github for my diary. I have no experience with this but I think it could be nice as a possibility to learn. Today i learnt how to do headers with one or tho ```#``` in order to make them in two different sizes. 
